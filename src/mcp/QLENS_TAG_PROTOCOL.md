...
QLens 的 `qlens_analyze`（AI 批量打标）在调用视觉模型前，会在程序内部对图片做**内存预压缩**，防止 AGENT 上传原图烧爆 token。

### 分辨率规格

| 任务 | 压缩目标 | 说明 |
|------|---------|------|
| **内容标签**（婚礼/风景/人物/物体） | **长边 ≤ 720px** | 视觉模型在 512-768px 训练，720px 已超过有效输入，再高无益 |
| **QC 质检**（模糊/曝光/色偏/闭眼/红眼） | 算法层在**原始分辨率**检测 | 降采样会误判模糊；QC 不经过 VLM |
| **细节 caption**（可选） | 长边 ≤ 1080px | 仅需要描述细节时提高 |

### 为什么 720px 足够

现代视觉模型（CLIP/ViT 类）按固定 patch 尺寸编码图像，超过训练分辨率的内容会被裁掉/缩小——**喂 4K 原图与喂 720p 看到的有效信息相同，token 却多花 90%+**。720p 长边是打标精度与成本的甜点。

### 规则

1. 压缩在 QLens 内部完成，**图片字节绝不进入 AGENT 上下文**
2. stable_id（hash）始终用**原始文件**计算（大小 + mtime + 前 4KB），与压缩无关
3. QC 检测在原始分辨率做算法分析，不压缩
